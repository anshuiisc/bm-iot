...

{
                ...
                taxiDetails= StringUtils.join(Arrays.copyOfRange(rowStringArray, 0, 3), ","); //TODO: ts to lat
                taxiID= rowStringArray[0];
                observedValArr = StringUtils.join(Arrays.copyOfRange(rowStringArray, 4, 17),",");  // temp to aq

                if(l.isInfoEnabled())
                l.info("taxiDetails : "+taxiDetails +"observedValArr"+observedValArr);

                collector.emit(new Values(taxiDetails,taxiID,observedValArr,msgId,"parse"));
...
//        outputFieldsDeclarer.declare(new Fields("RowString","MSGID","Res"));
        outputFieldsDeclarer.declare(new Fields("taxiMeta","taxiID","obsVal","MSGID","msgtype")); // obsType = {temp, humid, airq, light, dust}
    }
